...
int test48() {
    string dithered = "AB";
    vector<string> screen = {"CCCBBB", "CCCBBB", "CCCAAA", "CCCAAA", "CCCAAA", "CCCBBB"};
    DitherCounter* pObj = new DitherCounter();
    clock_t start = clock();
    int result = pObj->count(dithered, screen);
    clock_t end = clock();
...
/*******************************************************************************
*    Top Submission URL: 
*    http://community.topcoder.com/stat?c=problem_solution&cr=8384303&rd=5854&pm=1728
********************************************************************************
#include <algorithm> 
#include <iostream> 
#include <sstream> 
...
class ImageDithering { 
   ...
   for (i=0;i<n;i++) { 
      for (j=0;j<m;j++) 
         if (dithered.find(screen[i][j])<dithered.size()) num++;       
      } 
