...
*    Problem URL: http://community.topcoder.com/stat?c=problem_statement&pm=9942
*******************************************************************************/
#include <vector>
#include <list>
#include <map>
...
int test0() {
    ...
    CubeNets* pObj = new CubeNets();
    clock_t start = clock();
    string result = pObj->isCubeNet(figure);
    clock_t end = clock();
    delete pObj;
    string expected = "YES";
    if(result == expected) {
...
/*******************************************************************************
*    Top Submission URL: 
*    http://community.topcoder.com/stat?c=problem_solution&cr=22653044&rd=13508&pm=9942
********************************************************************************
#include "assert.h" 
#include "ctype.h" 
#include "float.h" 
...
Move move[MOVES_COUNT]; 
 
enum Face {TOP = 0, NORTH, WEST, EAST, SOUTH, BOTTOM}; 
bool cube[6]; 
int paper[6][6]; 
...
struct CubeNets { 
  string isCubeNet(vector<string> figure) { 
    const int m = (int)figure.size(), n = (int)figure[0].size(); 
    memset(paper, 0, sizeof(paper)); 
    for (int i = 0; i < m; ++i) for (int j = 0; j < n; ++j) 
      if (figure[i][j] == '#') paper[i][j] = 1; 
    InitMoves(); 
    return IsArmable() ? "YES" : "NO"; 
  } 
}; 
