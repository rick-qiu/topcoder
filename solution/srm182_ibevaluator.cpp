...
*    Problem URL: http://community.topcoder.com/stat?c=problem_statement&pm=2306
*******************************************************************************/
#include <vector>
#include <list>
#include <map>
...
int test0() {
    ...
    IBEvaluator* pObj = new IBEvaluator();
    clock_t start = clock();
    vector<int> result = pObj->getSummary(predictedGrades, actualGrades);
    clock_t end = clock();
    delete pObj;
...
/*******************************************************************************
*    Top Submission URL: 
*    http://community.topcoder.com/stat?c=problem_solution&cr=311563&rd=4730&pm=2306
********************************************************************************
#include <string>
#include <vector>
#include<math.h>
...
class IBEvaluator {
  ...
  vector <int> getSummary(vector <int> pred, vector <int> act) {
    ...
    for(i=0;i<size;i++)
    {
      int dif = abs(pred[i]-act[i]);
      diff[dif]++;
    }
    for(i=0;i<7;i++)
    {
      ret[i]=diff[i]*100.00/size;
    }
    return ret;
  }
};
